# API для всей железной дороги
# Весь класс является враппером для объектов железной дороги,
# Поэтому никакой приватной логики здесь нет
class RailRoad
  ...
  def add_station_to_route(station_number, route_number)
    ...
    route.add_station(station) if station != nil and route != nil
  end

  def remove_station_from_route(station_number, route_number)
    route = @routes[route_number - 1]
    station = route.stations[station_number - 1]
    route.remove_station(station) if station != nil and route != nil
  end

  def assign_train_on_route(train_number, route_number)
    train = @trains[train_number - 1]
    route = @routes[route_number - 1]
    train.assign_route(route) if train != nil and route != nil
  end

  def train_move_forward(train_number)
    train = @trains[train_number - 1]
    train.move_forward if train != nil and train.route != nil
  end

  def train_move_backward(train_number)
    train = @trains[train_number - 1]
    train.move_backward if train != nil and train.route != nil
  end
end